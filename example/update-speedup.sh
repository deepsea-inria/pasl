...
if [ $dorun -eq 1 ]
then
  prun speedup -runs $runs -baseline "rake-compress-update.opt -seq ${baseline}" -parallel "rake-compress-update.opt -seq 0 -proc 1,2,4,6,8,10,12,15,20,30,39" -type add -n 1000000 -graph binary_tree -k 1,100,1000,10000,30000,60000,100000,300000,999999 -output "plots/update_speedup/binary_tree_update_${baseline}.txt"
fi
...
if [ $dorun -eq 1 ]
then
  prun speedup -runs $runs -baseline "rake-compress-update.opt -seq ${baseline}" -parallel "rake-compress-update.opt -seq 0 -proc 1,2,4,6,8,10,12,15,20,30,39" -type add -n 1000000 -graph bamboo -k 1,10,100,1000,10000,30000,60000,100000,300000,999999 -output "plots/update_speedup/bamboo_update_${baseline}.txt"
fi
...
do
  if [ $dorun -eq 1 ]
  then
    prun speedup -runs $runs -baseline "rake-compress-update.opt -seq ${baseline}" -parallel "rake-compress-update.opt -seq 0 -proc 1,2,4,6,8,10,12,15,20,30,39" -type add -n 1000000 -graph random_tree -fraction ${f} -k 1,10,100,1000,10000,30000,60000,100000,300000,999999 -output "plots/update_speedup/random_${f}_update_${baseline}.txt"
  fi
