...
template <
  ...
  class Seq_convert_scan
>
controller_type scan<Input,Output,Result,Convert_reduce_comp,Convert_reduce,Convert_scan,Seq_convert_scan>::contr(
  "scan"+sota<Input>()+sota<Output>()+sota<Result>()+sota<Convert_reduce_comp>()+
...
void scan(Input& in,
          ...
          scan_type st) {
  using controller_type = contr::scan<Input, Output, Result, Convert_reduce_comp, Convert_reduce, Convert_scan, Seq_convert_scan>;
  const long k = Scan_branching_factor;
  long n = in.size();
  long m = get_nb_blocks(k, n);
  auto loop_comp = [&] (long i) {
...
void scan(Iter lo,
  ...
  auto convert_scan = [&] (input_type& in, typename parray::parray<Result>::iterator outs_lo) {
    long i = 0;
    level4::scan_seq(in.lo, in.hi, outs_lo, out, id, [&] (Iter src, Result& dst) {
      lift_idx_dst(i, src, dst);
    }, st);
  };
  auto seq_convert_scan = [&] (input_type& in, typename parray::parray<Result>::iterator outs_lo) {
