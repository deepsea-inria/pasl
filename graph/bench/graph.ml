...
let plot () =
   ...
   Mk_table.build_table tex_file pdf_file (fun add ->
    ...
    ~~ List.iter envs_tables (fun env_tables ->
       ...
       add (Latex.tabular_multicol 6 "|c||" (Latex.bold "BFS")); add " & ";
       add Latex.tabular_newline;
       add "graph & verti. & edges & max & seq & our & Cong. & \\bf{Cong} & seq & our & LS & \\bf{LS vs } & Ligra & Ligra[old] & our PDFS ";
       add Latex.new_line;
       add " &  (m) & (m) & dist & DFS & PDFS & PDFS & \\bf{vs ours} & BFS & PBFS & PBFS & \\bf{  ours } & vs ours & vs ours & vs PBFS";
       add Latex.tabular_newline;
       ~~ List.iter envs_rows (fun env_rows ->
         let results = Results.filter env_rows results in
         let results_baseline = Results.filter env_rows results_baseline in
...
let _ =
   ...
   let bindings = [
      ...
      "overview_old", ExpOverviewOld.all;
